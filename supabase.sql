...
drop policy if exists "members can view checklist" on public.checklist_items;
create policy "members can view checklist"
on public.checklist_items for select to authenticated
using (public.is_trip_member(trip_id));

drop policy if exists "members can add checklist" on public.checklist_items;
create policy "members can add checklist"
...
begin
  ...
  if trim(coalesce(p_name, '')) = '' then
    raise exception 'Escriu un nom per al viatge';
  end if;
